...
#	ssh -o StrictHostKeyChecking=no $netid@$hostname "cd $(pwd); java MainThread $root_node temp" &  #execute the java parser file and pass the args as node id and temp file
