...
do
	...
	if [ $node_id -ne $root_node ]; then
		echo $netid@$host "java MainThread 'temp' $node_id"
		ssh -o StrictHostKeyChecking=no $netid@$host "cd $(pwd); java MainThread $node_id temp" &
...
echo $root_hostName
sleep 5
ssh -o StrictHostKeyChecking=no $netid@$root_hostName "cd $(pwd); java MainThread temp $root_node" &
